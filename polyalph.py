...
#to be truly safe, no hints must be given
#CHARACTERS_THAT_MUST_REMAIN_THE_SAME = string.whitespace
...
t = input("Enter plaintext to encode/decode:\n> ")
while True:
    ans = input("Encode[e] or decode[d]? Press [q] to quit.\n")
    if ans.lower() == "encode" or ans.lower() == "e":
        k = input("Enter a key to use:\n>> ")
        print("Encrypted: "+encrypt(t, k))
        break
    elif ans.lower() == "decode" or ans.lower() == "d":
        y = input("Enter the key used:\n>> ")
        print("Decrypted: "+decrypt(t, y))
        break
    elif ans.lower() == "q":
        quit()
    else:
